...
/* package */ final class AudioTagPayloadReader extends TagPayloadReader {
  ...
  protected boolean parseHeader(ParsableByteArray data) throws UnsupportedFormatException {
    if (!hasParsedAudioDataHeader) {
      ...
      if (audioFormat == AUDIO_FORMAT_ALAW || audioFormat == AUDIO_FORMAT_ULAW) {
          ...
          int minSize = AudioTrack.getMinBufferSize(8000, AudioFormat.CHANNEL_OUT_MONO, encoding);

          Format format = Format.createAudioSampleFormat(null, type, null, minSize * 4, minSize,
  ...
  protected void parsePayload(ParsableByteArray data, long timeUs) {
    ...
    if (audioFormat != AUDIO_FORMAT_AAC || packetType == AAC_PACKET_TYPE_AAC_RAW) {
      int bytes = data.bytesLeft();
      output.sampleData(data, bytes);
      output.sampleMetadata(timeUs, C.BUFFER_FLAG_KEY_FRAME, bytes, 0, null);
    } else if (packetType == AAC_PACKET_TYPE_SEQUENCE_HEADER && !hasOutputFormat) {
